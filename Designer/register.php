...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        $username_err = "Kérlek, adj egy e-mail címet!";
    } elseif(!preg_match('/^[a-zA-Z0-9_@.]+$/', trim($_POST["username"]))){
        $username_err = "Kérlek, adj egy helyes e-mail címet.";
    } else{
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $username_err = "Ez az e-mail cím már foglalt.";
                } else{
                    $username = trim($_POST["username"]);
                }
            } else{
                echo "Hoppá! Valami nem ment ahogy kell. Kérjük, próbáld meg mégegyszer.";
            }
    ...
    if(empty(trim($_POST["password"]))){
        $password_err = "Kérlek, írj be egy jelszavat.";     
    } elseif(strlen(trim($_POST["password"])) < 6){
        $password_err = "A jelszavad legalább 6 karaktert kell tartalmazzon";
    } else{
    ...
    if(empty(trim($_POST["confirm_password"]))){
        $confirm_password_err = "Kérlek írd be a jelszavat mégegyszer.";     
    } else{
...
<div id="mySidenav" class="sidenav">
  ...
  <a href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a>
   <a href="login.php"> <img class="icon" src="mainElements\icons\login-white.svg"></img> Bejelentkezés
...
<?php
if(isset($_SESSION["loggedin"])){
				echo '<script> alertify.success("Kijelentkeztél!"); </script>';	
				$_SESSION = array();
					...
					<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
						<div class="form-group">
							...
							<span class="invalid-feedback"><?php 
															...
															{
															echo '<script> 	alertify.error("' . $username_err . '");</script>'; 
															}
															?></span>
						</div>    
						<div class="form-group">
							...
							<span class="invalid-feedback"><?php 
															...
															{
															echo '<script> 	alertify.error("' . $password_err . '");</script>'; 
															}
															?></span>
						</div>
						<div class="form-group">
							...
							<span class="invalid-feedback"><?php 
															...
															{
															echo '<script> 	alertify.error("' . $confirm_password_err . '");</script>'; 
															}
															?></span>
						...
						<div class="form-group">
							...
							<input type="reset" class="btn btn-secondary ml-2" value="Kiüresítés">
						</div>
